...
func SwiftFolderCopyTestdataFromProjectPath(project *vxproject, targetpath string) *vxmsgblock {
	msgblock := NewMsgBlock("SwiftFileCopyTestdataFromProjectPath")
	...
	for _, subproject := range project.listproject {
		msgs := SwiftFolderCopyTestdataFromProjectPath(subproject, targetpath)
		msgblock = MsgblockAddBlock(msgblock, msgs)
	}
	return msgblock
}

func SwiftConstDefsFromListConst(values []*vxconst, indent string) string {
	output := "null"
	lineindent := "\n" + indent
...
func SwiftDebugFromFunc(fnc *vxfunc, lineindent string) (string, string) {
	debugstart := ""
	debugend := ""
	if fnc.debug {
		debugstart = lineindent + "Core.debug(\"" + fnc.name + "\", \"start\""
		for _, arg := range fnc.listarg {
			debugstart += ", " + SwiftTypeStringValNew(arg.name+"=") + ", " + SwiftFromName(arg.alias)
		}
		debugstart += ");"
...
func SwiftDocFromFunc(fnc *vxfunc) string {
	var doc = ""
	var argsdoc = ""
	...
	doc += "@function " + SwiftFromName(fnc.alias)
	if fnc.idx > 0 {
...
func SwiftEmptyValueFromType(typ *vxtype) string {
	return SwiftEmptyValueFromTypeIndent(typ, "")
}

func SwiftEmptyValueFromTypeIndent(typ *vxtype, indent string) string {
	output := "\"\""
	if len(indent) < 10 {
		...
		case "string":
			output = "\"" + output + "\""
		case ":list":
			output = "Core.f_type_to_list(" + SwiftNameFromPkgName(typ.pkgname) + ".t_" + typ.name + ")"
		default:
			if len(typ.properties) > 0 {
				output = "{\n"
				for _, property := range typ.properties {
					propdefault := SwiftEmptyValueFromTypeIndent(property.vxtype, indent+"  ")
					output += indent + "    " + SwiftFromName(property.name) + ": " + propdefault + ","
					if property.doc != "" {
						output += " // " + property.doc
					}
					output += "\n"
				...
				output += "" +
					indent + "    vxtype: " + SwiftNameFromPkgName(typ.pkgname) + ".t_" + SwiftFromName(typ.name) +
					"\n" + indent + "  }"
...
func SwiftFilesFromProjectCmd(project *vxproject, command *vxcommand) ([]*vxfile, *vxmsgblock) {
	msgblock := NewMsgBlock("SwiftFilesFromProjectCmd")
	var files []*vxfile
	...
	case ":source":
		...
		file.name = "App.swift"
		file.path = cmdpath
		file.text = SwiftApp(project, command)
		files = append(files, file)
	case ":test":
		file := NewFile()
		file.name = "AppTest.swift"
		file.path = cmdpath
		file.text = SwiftAppTest(project, command)
		files = append(files, file)
		file = NewFile()
		file.name = "TestLib.swift"
		file.path = cmdpath
		file.text = SwiftTestLib()
		files = append(files, file)
	}
	...
	for _, pkg := range pkgs {
		...
		switch command.code {
		case ":source":
			text, msgs := SwiftFromPackage(pkg, project)
			msgblock = MsgblockAddBlock(msgblock, msgs)
			file := NewFile()
			file.name = pkgname + ".swift"
			file.path = cmdpath + "/" + pkgpath
			file.text = text
			files = append(files, file)
		case ":test":
			text, msgs := SwiftTestFromPackage(pkg, project)
			msgblock = MsgblockAddBlock(msgblock, msgs)
			file := NewFile()
			file.name = pkgname + "Test.swift"
			file.path = cmdpath + "/" + pkgpath
			file.text = text
			files = append(files, file)
		}
...
func SwiftFromConst(cnst *vxconst, pkg *vxpackage) (string, string, *vxmsgblock) {
	msgblock := NewMsgBlock("SwiftFromConst")
	var output = ""
	var doc = ""
	...
	doc += "{" + cnsttype.name + "}"
	cnstname := SwiftFromName(cnst.alias)
	cnstclassname := "Const_" + cnstname
	cnsttypename := SwiftNameFromType(cnst.vxtype)
	cnsttypeclassname := SwiftNameTypeFullFromType(cnsttype)
	initval := ""
	const_new := ""
	...
	default:
		...
		case ":list":
			clstext, msgs := SwiftFromValue(cnst.value, cnst.pkgname, emptyfunc, 3, true, false, path)
			msgblock = MsgblockAddBlock(msgblock, msgs)
			if clstext != "" {
		...
		case ":struct":
			...
			if clstext != "" {
				...
				for _, prop := range ListPropertyTraitFromType(cnst.vxtype) {
					const_new += "" +
						"\n      output.vx_p_" + SwiftFromName(prop.name) + " = val." + SwiftFromName(prop.name) + "();"

				}
			}
			/*
				clstext, msgs := SwiftFromValue(cnst.value, cnst.pkgname, emptyfunc, "        ", true, false, path)
				msgblock = MsgBlockAddBlock(msgblock, msgs)
				if clstext != "" {
					cnstval = "" +
	...
	extends := SwiftNameClassFullFromType(cnsttype)
	output += "" +
		"\n  /**" +
		"\n   * " + StringFromStringIndent(doc, "   * ") +
		"\n   */" +
...
func SwiftFromFunc(fnc *vxfunc) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("SwiftFromFunc")
	var listargtype []string
	var listargname []string
	funcname := SwiftFromName(fnc.alias) + SwiftIndexFromFunc(fnc)
	returntype := ""
	if fnc.generictype == nil {
		returntype = SwiftGenericFromType(fnc.vxtype)
	} else {
		returntype = SwiftGenericFromType(fnc.generictype)
	}
	pkgname := SwiftNameFromPkgName(fnc.pkgname)
	instancevars := ""
	staticvars := ""
	instancefuncs := ""
	staticfuncs := ""
	f_suppresswarnings := ""
	path := fnc.pkgname + "/" + fnc.name + SwiftIndexFromFunc(fnc)
	genericdefinition := SwiftGenericDefinitionFromFunc(fnc)
	if fnc.isgeneric {
		switch NameFromFunc(fnc) {
		case "vx/core/copy", "vx/core/empty", "vx/core/new":
		...
		default:
			if fnc.generictype != nil {
				genericargname := SwiftNameTFromTypeGeneric(fnc.generictype)
				argtext := "final " + SwiftGenericFromType(fnc.generictype) + " " + genericargname
				listargtype = append(listargtype, argtext)
	...
	default:
		for _, arg := range fnc.listarg {
			...
			if fnc.generictype != nil && argtype.isgeneric {
				argtypename = SwiftGenericFromType(argtype)
			} else {
				argtypename = SwiftNameTypeFromType(argtype)
			}
			argtext := "final " + argtypename + " " + SwiftFromName(arg.alias)
			listargname = append(listargname, SwiftFromName(arg.alias))
			if arg.multi {
				listargtype = append(listargtype, argtext)
			} else {
				listargtype = append(listargtype, argtext)
	...
	functypetext := ""
	if fnc.generictype != nil {
		functypetext = SwiftGenericFromType(fnc.generictype)
	} else {
	...
	default:
		...
		default:
			...
			case 1:
				argtypename := ""
				arg := fnc.listarg[0]
				argtypename = SwiftNameTypeFromType(arg.vxtype)
				subargnames := "inputval"
				switch NameFromFunc(fnc) {
				case "vx/core/empty":
				default:
					if fnc.generictype != nil {
						subargnames = SwiftNameTFromType(fnc.vxtype) + ", inputval"
					}
				}
				contextname := ""
				if fnc.context {
				...
				if fnc.async {
					...
					if issamegeneric {
						...
						asyncbody += "" +
							"\n      " + SwiftNameTypeFromType(arg.vxtype) + " inputval = Core.f_any_from_any(" + SwiftNameTFromType(arg.vxtype) + ", value);" +
	...
	repltext := SwiftReplFromFunc(fnc)
	instancefuncs += repltext
	valuetext, msgs := SwiftFromValue(fnc.value, fnc.pkgname, fnc, 0, true, false, path)
	msgblock = MsgblockAddBlock(msgblock, msgs)
	...
	permissionbottom := ""
	if fnc.permission {
		permissiontop = lineindent + "if (Core.f_boolean_permission_from_func(" + SwiftNameTFromFunc(fnc) + ", context).vx_boolean()) {"
		permissionbottom = "" +
			lineindent + "} else {" +
	...
	interfacefn := SwiftInterfaceFnFromFunc(fnc)
	debugtop, debugbottom := SwiftDebugFromFunc(fnc, lineindent)
	switch NameFromFunc(fnc) {
	case "vx/core/new":
		f_suppresswarnings = "\n  @SuppressWarnings(\"unchecked\")"
	}
	...
	default:
		if fnc.vxtype.name == "none" {
			...
			defaultvalue = lineindent + "CompletableFuture<" + returntype + "> output = Core.async_new_completed(" + SwiftNameEFromType(fnc.vxtype) + ");"
		} else if fnc.generictype != nil {
			defaultvalue = lineindent + returntype + " output = Core.f_empty(" + SwiftNameTFromTypeGeneric(fnc.generictype) + ");"
		} else {
			defaultvalue = lineindent + returntype + " output = " + SwiftNameEFromType(fnc.vxtype) + ";"
		}
	}
	interfacefunc := SwiftInterfaceFromFunc(fnc)
	doc := SwiftDocFromFunc(fnc)
	output := "" +
		doc +
		interfacefunc +
		...
		"\n        " + SwiftTypeDefFromType(fnc.vxtype, "        ") + " // typedef" +
		"\n      );" +
		"\n    }" +
		"\n" +
		"\n    @Override" +
		...
		"\n  }" +
		"\n" +
		"\n  public static final Func_" + funcname + " e_" + funcname + " = new " + SwiftNameFromPkgName(fnc.pkgname) + ".Class_" + funcname + "();" +
		"\n  public static final Func_" + funcname + " t_" + funcname + " = new " + SwiftNameFromPkgName(fnc.pkgname) + ".Class_" + funcname + "();" +
		"\n" +
		f_suppresswarnings +
...
func SwiftFuncDefsFromFuncs(funcs []*vxfunc, indent string) string {
	output := "null"
	lineindent := "\n" + indent
	if len(funcs) > 0 {
...
func SwiftFromName(name string) string {
	output := name
	if output == "extends" {
		output = "extend"
	}
...
func SwiftFromPackage(pkg *vxpackage, prj *vxproject) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("SwiftFromPackage")
	pkgpath, pkgname := SwiftPackagePathFromPrefixName(pkg.name)
	specialcode := prj.maptext[pkg.name+"_swift.txt"]
	typkeys := ListKeyFromMapType(pkg.maptype)
	typetexts := ""
	for _, typid := range typkeys {
		typ := pkg.maptype[typid]
		typetext, msgs := SwiftFromType(typ)
		msgblock = MsgblockAddBlock(msgblock, msgs)
		typetexts += typetext
	}
	...
	for _, cnstid := range cnstkeys {
		...
		consttext, constlate, msgs := SwiftFromConst(cnst, pkg)
		msgblock = MsgblockAddBlock(msgblock, msgs)
	...
	for _, fncid := range fnckeys {
		...
		for _, fnc := range fncs {
			fnctext, msgs := SwiftFromFunc(fnc)
			msgblock = MsgblockAddBlock(msgblock, msgs)
			functexts += fnctext
		}
	}
	body := "" +
		...
		constlates
		//		emptytypes
	imports := SwiftImportsFromPackage(pkg, body, false)
	output := "" +
		"package " + pkgpath + ";" +
		"\n" +
		imports +
...
func SwiftFromText(text string) string {
	var output = text
...
func SwiftFromType(typ *vxtype) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("SwiftFromType")
	output := ""
	...
	typename := SwiftFromName(typ.alias)
	instancefuncs := ""
	createtext, msgs := SwiftFromValue(typ.createvalue, "", emptyfunc, 2, true, false, path)
	msgblock = MsgblockAddBlock(msgblock, msgs)
	if createtext != "" {
		instancefuncs += "\n    " + createtext + "\n"
	}
	...
	case ":list":
		...
		if len(allowtypes) > 0 {
			...
			allowclass = SwiftNameTypeFullFromType(allowtype)
			allowempty := SwiftNameEFromType(allowtype)
			allowname = SwiftNameFromType(allowtype)
			allowcode = "" +
				"\n    @Override" +
		...
		for _, allowedtype := range typ.allowtypes {
			allowedtypename := SwiftNameTypeFromType(allowedtype)
	...
	case ":struct":
		...
		default:
			...
			for _, arg := range props {
				...
				argname := SwiftFromName(arg.name)
				...
				vx_any += "" +
					...
					"\n        output = this." + argname + "();" +
					"\n        break;"
				argclassname := SwiftNameTypeFromType(arg.vxtype)
				argalt := ""
				...
				instancefuncs += "" +
					...
					"\n      return this.vx_p_" + argname + " == null ? " + SwiftNameEFromType(arg.vxtype) + " : this.vx_p_" + argname + ";" +
					//"\n      Core.Type_any valsub = this.vx_map().getOrDefault(\":" + arg.name + "\", " + SwiftNameEFromType(arg.vxtype) + ");" +
					//"\n      return Core.f_any_from_any(" + SwiftNameTFromType(arg.vxtype) + ", valsub);" +
					"\n    }" +
					"\n"
			}
			defaultkey := ""
			...
			if lastarg.isdefault {
				lastargname := SwiftFromName(lastarg.name)
				argclassname := SwiftNameTypeFromType(lastarg.vxtype)
				defaultkey += "" +
				...
				if lastarg.vxtype.extends == ":list" {
					for _, allowtype := range lastarg.vxtype.allowtypes {
						subargclassname := SwiftNameTypeFromType(allowtype)
						defaultkey += "" +
							...
							"\n            if (vallist == null) {" +
							"\n              vallist = " + SwiftNameTFromType(lastarg.vxtype) + ".vx_new(valdefault);" +
							"\n            } else {" +
	...
	sinterface := SwiftInterfaceFromType(typ)
	typedef := "" +
		"\n    @Override" +
		"\n    public Core.Type_typedef vx_typedef() {" +
		"\n      return " + SwiftTypeDefFromType(typ, "      ") + ";" +
		"\n    }" +
		"\n"
	output += "" +
		"\n  /**" +
		...
		"\n   */" +
		sinterface +
		"\n  public static class Class_" + typename + extendsclass + " implements Type_" + SwiftNameFromType(typ) + " {" +
		"\n" +
		instancefuncs +
		"\n    @Override" +
...
func SwiftFromValue(value vxvalue, pkgname string, parentfn *vxfunc, indent int, encode bool, test bool, path string) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("SwiftFromValue")
	var output = ""
	valstr := ""
	...
	case ":arg":
		...
		valstr = SwiftFromName(valarg.name)
		output += valstr
	case ":const":
		switch value.name {
		case "false", "true":
		...
		default:
			if value.pkg == ":native" {
				valstr = SwiftFromName(value.name)
			} else {
				valconst := ConstFromValue(value)
				valstr = SwiftNameFromPkgName(valconst.pkgname) + ".c_" + SwiftFromName(valconst.alias)
			}
		}
		output = valstr
	case ":func":
			...
			for _, arg := range fnc.listarg {
				...
				if valuetext == ":swift" {
					...
					if argvalue.name == "newline" {
						...
						clstext, msgs := SwiftFromValue(argvalue, pkgname, parentfn, 0, false, test, subpath)
						msgblock = MsgblockAddBlock(msgblock, msgs)
						argtext = clstext
		...
		default:
			...
			case "vx/core/any<-struct":
				...
				case "string":
					...
					work, msgs := SwiftFromValue(structvalue, pkgname, fnc, 0, true, test, subpath)
					msgblock = MsgblockAddBlock(msgblock, msgs)
					work = work + "." + SwiftFromName(propname) + "()"
			...
			case "vx/core/let":
				if fnc.async {
					output += SwiftNameFromPkgName(fnc.pkgname) + ".f_let_async("
				} else {
					output += SwiftNameFromPkgName(fnc.pkgname) + ".f_" + SwiftNameFromFunc(fnc) + "("
				}
			default:
				output += SwiftNameFromPkgName(fnc.pkgname) + ".f_" + SwiftNameFromFunc(fnc) + "("
			}
			if !isskip {
				if fnc.isgeneric {
					...
					default:
						if fnc.generictype != nil {
							genericarg := SwiftNameTFromTypeGeneric(fnc.vxtype)
					...
					if fncidx == 0 && funcname == "vx/core/fn" {
						...
						if argvalue.code == ":func" && argvalue.name == "fn" {
							...
							for _, lambdaarg := range arglist {
								...
								argvaltype = SwiftNameTypeFullFromType(argtype)
								argvalinstance = SwiftNameTFromType(argtype)
							...
							if argfunc.async {
								work, msgs := SwiftFromValue(argvalue, pkgname, fnc, subindent, true, test, argsubpath)
								msgblock = MsgblockAddBlock(msgblock, msgs)
								work = "\n  return " + work + ";"
								...
								case "any<-key-value-async",
									...
									argtext = "" +
										SwiftNameTFromType(funcarg.vxtype) + ".vx_fn_new((" + lambdatext + ") -> {" +
										lambdavartext +
										work +
										"\n})"
								default:
							...
							case 1:
								...
								if argasync {
									...
									for _, lambdaarg := range arglist {
										...
										if lambdaarg.async {
											lambdavaluetext, msgs := SwiftFromValue(lambdaarg.value, pkgname, fnc, argindent, true, test, argsubpath)
											msgblock = MsgblockAddBlock(msgblock, msgs)
											lambdatext += "" +
												arglineindent + "final CompletableFuture<" + SwiftNameTypeFromType(lambdaarg.vxtype) + "> future_" + SwiftFromName(lambdaarg.name) + " = " + lambdavaluetext + ";" +
												arglineindent + "return Core.async_from_async_fn(future_" + SwiftFromName(lambdaarg.name) + ", (" + SwiftFromName(lambdaarg.name) + ") -> {"
											aftertext += "" +
												arglineindent + "});"
											argindent += 1
										} else {
											lambdavaluetext, msgs := SwiftFromValue(lambdaarg.value, pkgname, fnc, argindent, true, test, argsubpath)
											msgblock = MsgblockAddBlock(msgblock, msgs)
											lambdatext += arglineindent + "final " + SwiftNameTypeFromType(lambdaarg.vxtype) + " " + SwiftFromName(lambdaarg.name) + " = " + lambdavaluetext + ";"
										}
									}
									work, msgs := SwiftFromValue(argvalue, pkgname, fnc, 2, true, test, argsubpath)
									msgblock = MsgblockAddBlock(msgblock, msgs)
									argtext = "" +
									...
									for _, lambdaarg := range arglist {
										...
										lambdavaluetext, msgs := SwiftFromValue(lambdaarg.value, pkgname, fnc, argindent, true, test, lambdaargpath)
										msgblock = MsgblockAddBlock(msgblock, msgs)
										lambdatext += arglineindent + "final " + SwiftNameTypeFromType(lambdaarg.vxtype) + " " + SwiftFromName(lambdaarg.name) + " = " + lambdavaluetext + ";"
									}
									work, msgs := SwiftFromValue(argvalue, pkgname, fnc, 0, true, test, argsubpath)
									msgblock = MsgblockAddBlock(msgblock, msgs)
							...
							case ":funcref":
								...
								case "any<-key-value", "any<-key-value-async",
									...
									lambdatext, lambdavartext, lambdaargtext := SwiftLambdaFromArgList(funcarglist, funcargfunc.isgeneric)
									work := SwiftNameFFromFunc(funcargfunc) + "(" + lambdaargtext + ")"
							...
							default:
								...
								work, msgs := SwiftFromValue(argvalue, pkgname, fnc, workindent, true, test, argsubpath)
								if converttoasync {
									sworkindent := StringRepeat("  ", workindent)
									work = "Core.f_async(" + SwiftNameTFromType(argfunctype) + ",\n" + sworkindent + work + "\n  )"
								}
								msgblock = MsgblockAddBlock(msgblock, msgs)
								...
								argtext = "" +
									SwiftNameTFromType(funcarg.vxtype) + ".vx_fn_new(() -> {" +
									work +
									"\n})"
							}
						}
						...
						if funcarg.multi {
							...
							if ismultiarg {
								...
								multiflag = true
								argtext = "" +
									SwiftNameTFromType(funcarg.vxtype) + ".vx_new(" +
	...
	case ":funcref":
		...
		valstr = ""
		valstr += SwiftNameFromPkgName(valfunc.pkgname) + ".t_" + SwiftFromName(valfunc.alias)
		output = sindent + valstr
	case ":type":
		valtype := TypeFromValue(value)
		output = SwiftNameFromPkgName(valtype.pkgname) + ".t_" + SwiftFromName(valtype.alias)
	case "string":
		...
		if encode {
			output = SwiftTypeStringValNew(output)
		}
	case "boolean":
		if encode {
			valstr = StringValueFromValue(value)
...
func SwiftGenericDefinitionFromFunc(fnc *vxfunc) string {
	output := ""
	var mapgeneric = make(map[string]string)
	if fnc.generictype != nil {
		returntype := SwiftGenericFromType(fnc.generictype)
		mapgeneric[fnc.vxtype.name] = returntype + " extends " + SwiftNameTypeFromType(fnc.vxtype)
		for _, arg := range fnc.listarg {
			argtype := arg.vxtype
			if !argtype.isfunc {
				if argtype.isgeneric {
					...
					if !ok {
						argtypename := SwiftGenericFromType(argtype)
						worktext := argtypename + " extends " + SwiftNameTypeFromType(arg.vxtype)
...
func SwiftGenericFromType(typ *vxtype) string {
	output := ""
	if typ.isgeneric {
		switch typ.name {
		case "any-1":
		...
		output = SwiftNameTypeFromType(typ)
	}
	return output
}

func SwiftImportsFromPackage(pkg *vxpackage, body string, test bool) string {
	output := ""
...
func SwiftIndexFromFunc(fnc *vxfunc) string {
	return StringIndexFromFunc(fnc)
}

func SwiftInterfaceFromType(typ *vxtype) string {
	output := ""
	typename := SwiftNameTypeFullFromType(typ)
	basics := "" +
	...
	createtext, _ := SwiftFromValue(typ.createvalue, "", emptyfunc, 0, true, false, "")
	if createtext != "" {
	...
	case "vx/core/boolean":
		output = "" +
			"\n  public protocol Type_boolean extends Core.Type_any {" +
			basics +
			"\n  }" +
			"\n"
	case "vx/core/decimal":
		output = "" +
			"\n  public protocol Type_decimal extends Core.Type_number {" +
			basics +
			"\n  }" +
			"\n"
	case "vx/core/float":
		output = "" +
			"\n  public protocol Type_float extends Core.Type_number {" +
			basics +
			"\n  }" +
			"\n"
	case "vx/core/func":
		output = "" +
			"\n  public protocol Type_func extends Core.Type_any {" +
			basics +
			"\n	   public Core.Type_funcdef vx_funcdef();" +
			"\n  }" +
			"\n"
	case "vx/core/int":
		output = "" +
			"\n  public protocol Type_int extends Core.Type_number {" +
			basics +
			"\n  }" +
			"\n"
	case "vx/core/string":
		output = "" +
			"\n  public protocol Type_string extends Core.Type_any {" +
			basics +
			"\n  }" +
			"\n"
	case "vx/core/list":
		output = "" +
			"\n  public protocol Type_list extends Core.Type_any {" +
			basics +
	...
	case "vx/core/map":
		output = "" +
			"\n  public protocol Type_map extends Core.Type_any {" +
			basics +
	...
	case "vx/core/struct":
		output = "" +
			"\n  public protocol Type_struct extends Core.Type_any {" +
			basics +
			"\n    public Core.Type_any vx_any(final Core.Type_string key);" +
			"\n    public Map<String, Core.Type_any> vx_map();" +
			"\n  }" +
		...
		case ":struct":
			...
			if len(typ.traits) > 0 {
				...
				for _, trait := range typ.traits {
					traitnames = append(traitnames, SwiftNameTypeFullFromType(trait))
				}
				extends = StringFromListStringJoin(traitnames, ", ")
			}
			for _, arg := range ListPropertyTraitFromType(typ) {
				extras += "\n    public " + SwiftNameTypeFromType(arg.vxtype) + " " + SwiftFromName(arg.alias) + "();"
			}
		default:
			extends += "Core.Type_any"
		}
		typename := SwiftNameFromType(typ)
		output = "" +
			"\n  public protocol Type_" + typename + " extends " + extends + " {" +
			basics +
			extras +
			"\n  }" +
			"\n"
...
func SwiftInterfaceFnFromFunc(fnc *vxfunc) string {
	interfaces := ""
	switch NameFromFunc(fnc) {
	case "vx/core/any<-any", "vx/core/any<-any-async",
		"vx/core/any<-any-context", "vx/core/any<-any-context-async",
		...
		for _, arg := range fnc.listarg {
			argtype := arg.vxtype
			argname := SwiftFromName(arg.alias)
			argtypename := SwiftNameTypeFromType(argtype)
...
func SwiftInterfaceFromFunc(fnc *vxfunc) string {
	funcname := SwiftNameFromFunc(fnc)
	extends := ""
	interfaces := ""
	generictypes := SwiftGenericDefinitionFromFunc(fnc)
	returntype := SwiftNameTypeFromType(fnc.vxtype)
	...
	output := "" +
		"\n  public static protocol Func_" + funcname + " extends " + extends + " {" +
		interfaces +
		"\n  }" +
		"\n"
	return output
...
func SwiftLambdaFromArgList(arglist []vxarg, isgeneric bool) (string, string, string) {
	...
	for _, lambdaarg := range arglist {
		argvaltype := ""
		argtype := lambdaarg.vxtype
		lambdaargname := SwiftFromName(lambdaarg.alias)
		...
		default:
			argvaltype = SwiftNameTypeFullFromType(argtype)
			argvaltname := SwiftNameTFromType(argtype)
...
func SwiftNameClassFullFromType(typ *vxtype) string {
	name := SwiftNameFromPkgName(typ.pkgname)
	name += ".Class_"
	name += SwiftNameFromType(typ)
	return name
}

func SwiftNameEFromType(typ *vxtype) string {
	output := ""
	if typ.isgeneric {
		output = "Core.f_empty(generic_" + SwiftFromName(typ.name) + ")"
	} else {
		output = "e_" + SwiftNameFromType(typ)
		if typ.pkgname != "" {
			output = SwiftNameFromPkgName(typ.pkgname) + "." + output
		}
	}
	return output
}

func SwiftNameFFromFunc(fnc *vxfunc) string {
	name := "f_" + SwiftNameFromFunc(fnc)
	if fnc.pkgname != "" {
		name = SwiftNameFromPkgName(fnc.pkgname) + "." + name
	}
	return name
}

func SwiftNameFromFunc(fnc *vxfunc) string {
	return SwiftFromName(fnc.alias) + SwiftIndexFromFunc(fnc)
}

func SwiftNameFromPkgName(pkgname string) string {
	output := ""
...
func SwiftNameFromType(typ *vxtype) string {
	name := ""
	if typ.alias == "" {
		name += SwiftFromName(typ.name)
	} else {
		name += SwiftFromName(typ.alias)
	}
	return name
}

func SwiftNameTFromFunc(fnc *vxfunc) string {
	name := "t_" + SwiftNameFromFunc(fnc)
	if fnc.pkgname != "" {
		name = SwiftNameFromPkgName(fnc.pkgname) + "." + name
	}
	return name
...
func SwiftNameTFromType(typ *vxtype) string {
	name := "t_" + SwiftNameFromType(typ)
	if typ.pkgname != "" {
		name = SwiftNameFromPkgName(typ.pkgname) + "." + name
	}
	return name
}

func SwiftNameTFromTypeGeneric(typ *vxtype) string {
	name := ""
	if typ.isgeneric {
		name = "generic_" + SwiftFromName(typ.name)
	} else {
		name = SwiftNameTFromType(typ)
	}
	return name
}

func SwiftNameTypeFromType(typ *vxtype) string {
	name := SwiftNameTypeFullFromType(typ)
	switch name {
	case "Core.Type_none":
		name = "void"
	}
...
func SwiftNameTypeFullFromType(typ *vxtype) string {
	name := SwiftNameFromPkgName(typ.pkgname)
	if typ.isfunc {
		name += ".Func_"
	} else {
...
func SwiftPackagePathFromPrefixName(pkgname string) (string, string) {
	pkgpath := pkgname
	...
	pkgpath = SwiftFromName(pkgpath)
	name = SwiftNameFromPkgName(name)
	return pkgpath, name
}

func SwiftReplFromFunc(fnc *vxfunc) string {
	output := ""
	replparams := ""
	argidx := 0
	var listargname []string
	...
	default:
		outputtype = SwiftNameTypeFromType(fnc.vxtype)
		outputttype = SwiftNameTFromType(fnc.vxtype)
		//emptytype = SwiftNameEFromType(fnc.vxtype)
		returnvalue = "output = "
	}
	if fnc.isgeneric {
		switch NameFromFunc(fnc) {
		...
		default:
			if fnc.generictype != nil {
				replparam := outputtype + " generic_" + SwiftFromName(fnc.generictype.name) + " = Core.f_any_from_any(" + outputttype + ", arglist.vx_any(Core.vx_new_int(" + StringFromInt(argidx) + ")));"
				replparams += "\n      " + replparam
				listargname = append(listargname, "generic_"+SwiftFromName(fnc.generictype.name))
			}
		}
	}
	for _, arg := range fnc.listarg {
		if (funcname == "let" || funcname == "let_async") && arg.name == "args" {
			...
			argname := SwiftFromName(arg.alias)
			replparam := SwiftNameTypeFromType(arg.vxtype) + " " + argname + " = Core.f_any_from_any(" + SwiftNameTFromType(arg.vxtype) + ", arglist.vx_any(Core.vx_new_int(" + StringFromInt(argidx) + ")));"
			replparams += "\n      " + replparam
...
func SwiftStringFromProjectCmd(prj *vxproject, cmd *vxcommand) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("SwiftStringFromProjectCmd")
	files, msgs := SwiftFilesFromProjectCmd(prj, cmd)
	msgblock = MsgblockAddBlock(msgblock, msgs)
	text := StringFromListFile(files)
	return text, msgblock
}

func SwiftTestCase(testvalues []vxvalue, testpkg string, testname string, testcasename string, fnc *vxfunc, path string) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("SwiftTestCase")
	var output = ""
	if len(testvalues) > 0 {
		var desctexts []string
		for idx, testvalue := range testvalues {
			subpath := path + "/tests" + StringFromInt(idx+1)
			descvaluetext, msgs := SwiftFromValue(testvalue, testpkg, fnc, 6, true, true, subpath)
			msgblock = MsgblockAddBlock(msgblock, msgs)
			desctext := "" +
				"\n        Test.t_testdescribe.vx_new(" +
				"\n          \":describename\", \"" + SwiftTestFromValue(testvalue) + "\"," +
				"\n          \":testresult\"," +
				"\n            " + descvaluetext +
				"\n        )"
...
func SwiftTestFromConst(cnst *vxconst) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("SwiftTestFromConst")
	testvalues := cnst.listtestvalue
	testpkg := cnst.pkgname
	testname := cnst.name
	testcasename := "c_" + SwiftFromName(cnst.alias)
	path := cnst.pkgname + "/" + cnst.name
	fnc := emptyfunc
	output, msgs := SwiftTestCase(testvalues, testpkg, testname, testcasename, fnc, path)
	msgblock = MsgblockAddBlock(msgblock, msgs)
	return output, msgblock
}

func SwiftTestFromFunc(fnc *vxfunc) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("SwiftTestFromFunc")
	testvalues := fnc.listtestvalue
	testpkg := fnc.pkgname
	idx := SwiftIndexFromFunc(fnc)
	testname := fnc.name + idx
	funcname := SwiftFromName(fnc.alias) + idx
...
func SwiftTestFromPackage(pkg *vxpackage, prj *vxproject) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("SwiftTestFromPackage")
	pkgpath, pkgname := SwiftPackagePathFromPrefixName(pkg.name)
	typkeys := ListKeyFromMapType(pkg.maptype)
	var coverdoccnt = 0
	var coverdoctotal = 0
	...
	for _, typid := range typkeys {
		covertypetotal += 1
		typ := pkg.maptype[typid]
		test, msgs := SwiftTestFromType(typ)
		msgblock = MsgblockAddBlock(msgblock, msgs)
		covertype = append(covertype, "      \":"+typid+"\", "+StringFromInt(len(typ.testvalues)))
		if test != "" {
			covertypecnt += 1
			typetexts += test
			testall = append(testall, pkgname+"Test.t_"+SwiftFromName(typ.alias)+"(context)")
		}
		coverdoctotal += 1
		if typ.doc != "" {
			coverdoccnt += 1
	...
	for _, cnstid := range cnstkeys {
		coverconsttotal += 1
		cnst := pkg.mapconst[cnstid]
		test, msgs := SwiftTestFromConst(cnst)
		msgblock = MsgblockAddBlock(msgblock, msgs)
		coverconst = append(coverconst, "      \":"+cnstid+"\", "+StringFromInt(len(cnst.listtestvalue)))
		if test != "" {
			coverconstcnt += 1
			consttexts += test
			testall = append(testall, pkgname+"Test.c_"+SwiftFromName(cnst.alias)+"(context)")
		}
		coverdoctotal += 1
		if cnst.doc != "" {
			coverdoccnt += 1
	...
	for _, fncid := range fnckeys {
		...
		fncs := pkg.mapfunc[fncid]
		for _, fnc := range fncs {
			test, msgs := SwiftTestFromFunc(fnc)
			msgblock = MsgblockAddBlock(msgblock, msgs)
			msgblock = MsgblockAddBlock(msgblock, msgs)
			coverfunc = append(coverfunc, "      \":"+fncid+SwiftIndexFromFunc(fnc)+"\", "+StringFromInt(len(fnc.listtestvalue)))
			if test != "" {
				coverfunccnt += 1
				functexts += test
				testall = append(testall, pkgname+"Test.f_"+SwiftFromName(fnc.alias)+SwiftIndexFromFunc(fnc)+"(context)")
			}
			coverdoctotal += 1
			if fnc.doc != "" {
				coverdoccnt += 1
	...
	body := "" +
		...
		"\n      \":testpkg\",   \"" + pkg.name + "\", " +
		"\n      \":constnums\", " + SwiftTypeCoverageNumsValNew(coverconstpct, coverconstcnt, coverconsttotal) + ", " +
		"\n      \":docnums\", " + SwiftTypeCoverageNumsValNew(coverdocpct, coverdoccnt, coverdoctotal) + ", " +
		"\n      \":funcnums\", " + SwiftTypeCoverageNumsValNew(coverfuncpct, coverfunccnt, coverfunctotal) + ", " +
		"\n      \":ospacenums\", " + SwiftTypeCoverageNumsValNew(coverbigospacepct, coverbigospacecnt, coverbigospacetotal) + ", " +
		"\n      \":otimenums\", " + SwiftTypeCoverageNumsValNew(coverbigotimepct, coverbigotimecnt, coverbigotimetotal) + ", " +
		"\n      \":totalnums\", " + SwiftTypeCoverageNumsValNew(coverpct, covercnt, covertotal) + ", " +
		"\n      \":typenums\", " + SwiftTypeCoverageNumsValNew(covertypepct, covertypecnt, covertypetotal) +
		"\n    );" +
		"\n  }" +
		"\n" +
		...
		"\n  }" +
		"\n"
	imports := SwiftImportsFromPackage(pkg, body, true)
	output := "" +
		"\npackage " + pkgpath + ";" +
		"\n" +
		imports +
...
func SwiftTestFromType(typ *vxtype) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("SwiftTestFromType")
	testvalues := typ.testvalues
	testpkg := typ.pkgname
	testname := typ.name
	testcasename := "t_" + SwiftFromName(typ.alias)
	fnc := emptyfunc
	path := typ.pkgname + "/" + typ.name
	output, msgs := SwiftTestCase(testvalues, testpkg, testname, testcasename, fnc, path)
	msgblock = MsgblockAddBlock(msgblock, msgs)
...
func SwiftTestFromValue(value vxvalue) string {
	var output = ""
	output = SwiftFromText(value.textblock.text)
	return output
}

func SwiftTypeCoverageNumsValNew(pct int, tests int, total int) string {
	return "" +
...
func SwiftTypeDefFromType(typ *vxtype, indent string) string {
	lineindent := "\n" + indent
	allowtypes := SwiftTypeListFromListType(typ.allowtypes)
	disallowtypes := SwiftTypeListFromListType(typ.disallowtypes)
	...
	traits := SwiftTypeListFromListType(typ.traits)
	output := "" +
		"Core.typedef_new(" +
...
func SwiftTypeDefMapFromProperties(args []vxarg, indent string) string {
	output := "null"
	lineindent := "\n" + indent
	if len(args) > 0 {
		var props []string
		for _, arg := range args {
			key := SwiftFromName(arg.alias)
			typ := SwiftTypeDefFromType(arg.vxtype, indent+"  ")
...
func SwiftTypeIntValNew(val int) string {
	return "Core.vx_new_int(" + StringFromInt(val) + ")"
}

func SwiftTypeListFromListType(listtype []*vxtype) string {
	output := "Core.e_typelist"
	if len(listtype) > 0 {
		var listtext []string
		for _, typ := range listtype {
			typetext := SwiftNameTFromType(typ)
...
func SwiftTypeStringValNew(val string) string {
	valstr := StringFromStringFindReplace(val, "\n", "\\n")
	return "Core.vx_new_string(\"" + valstr + "\")"
}

func SwiftWriteFromProjectCmd(prj *vxproject, cmd *vxcommand) *vxmsgblock {
	msgblock := NewMsgBlock("SwiftWriteFromProjectCmd")
	files, msgs := SwiftFilesFromProjectCmd(prj, cmd)
	msgblock = MsgblockAddBlock(msgblock, msgs)
	...
	case ":test":
		...
		msgs := SwiftFolderCopyTestdataFromProjectPath(prj, targetpath)
		msgblock = MsgblockAddBlock(msgblock, msgs)
	}
	return msgblock
}

func SwiftApp(project *vxproject, cmd *vxcommand) string {
	includetext := ""
	contexttext := `
	...
	if cmd.context == "" && cmd.main == "" {
		...
		if contextfunc != emptyfunc {
			...
			if contextfunc.async {
				contexttext = `
      CompletableFuture<Core.Type_context> asynccontext = ` + SwiftNameFFromFunc(contextfunc) + `(arglist);
      Core.Type_context context = Core.vx_sync_from_async(vx_core::t_context, asynccontext);`
			} else {
				contexttext = `
      Core.Type_context context = ` + SwiftNameFFromFunc(contextfunc) + `(arglist);`
			}
		}
		if mainfunc != emptyfunc {
			...
			mainfunctext := SwiftNameFFromFunc(mainfunc) + "(" + params + ");"
			if mainfunc.async {
				maintext = `
...
func SwiftAppTest(project *vxproject, cmd *vxcommand) string {
	imports := ""
	contexttext := `
	...
	if cmd.context == "" {
		...
		if contextfunc != emptyfunc {
			...
			if contextfunc.async {
				contexttext = `
  Core.Type_anylist arglist = Core.e_anylist;
  CompletableFuture<Core.Type_context> asynccontext = ` + pkgpath + `.` + SwiftNameFFromFunc(contextfunc) + `(arglist);
  Core.Type_context context = Core.vx_sync_from_async(vx_core::t_context, asynccontext);`
			} else {
				contexttext = `
  Core.Type_anylist arglist = Core.e_anylist;
  Core.Type_context context = ` + pkgpath + `.` + SwiftNameFFromFunc(contextfunc) + `(arglist);`
			}
		}
	}
	tests := ""
