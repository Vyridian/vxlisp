...
func KotlinFolderCopyTestdataFromProjectPath(project *vxproject, targetpath string) *vxmsgblock {
	msgblock := NewMsgBlock("KotlinFileCopyTestdataFromProjectPath")
	...
	for _, subproject := range project.listproject {
		msgs := KotlinFolderCopyTestdataFromProjectPath(subproject, targetpath)
		msgblock = MsgblockAddBlock(msgblock, msgs)
	}
	return msgblock
}

func KotlinConstDefsFromListConst(values []*vxconst, indent string) string {
	output := "null"
	lineindent := "\n" + indent
...
func KotlinDebugFromFunc(fnc *vxfunc, lineindent string) (string, string) {
	debugstart := ""
	debugend := ""
	if fnc.debug {
		debugstart = lineindent + "Core.debug(\"" + fnc.name + "\", \"start\""
		for _, arg := range fnc.listarg {
			debugstart += ", " + KotlinTypeStringValNew(arg.name+"=") + ", " + KotlinFromName(arg.alias)
		}
		debugstart += ");"
...
func KotlinDocFromFunc(fnc *vxfunc) string {
	var doc = ""
	var argsdoc = ""
	...
	doc += "@function " + KotlinFromName(fnc.alias)
	if fnc.idx > 0 {
...
func KotlinEmptyValueFromType(typ *vxtype) string {
	return KotlinEmptyValueFromTypeIndent(typ, "")
}

func KotlinEmptyValueFromTypeIndent(typ *vxtype, indent string) string {
	output := "\"\""
	if len(indent) < 10 {
		...
		case "string":
			output = "\"" + output + "\""
		case ":list":
			output = "Core.f_type_to_list(" + KotlinNameFromPkgName(typ.pkgname) + ".t_" + typ.name + ")"
		default:
			if len(typ.properties) > 0 {
				output = "{\n"
				for _, property := range typ.properties {
					propdefault := KotlinEmptyValueFromTypeIndent(property.vxtype, indent+"  ")
					output += indent + "    " + KotlinFromName(property.name) + ": " + propdefault + ","
					if property.doc != "" {
						output += " // " + property.doc
					}
					output += "\n"
				...
				output += "" +
					indent + "    vxtype: " + KotlinNameFromPkgName(typ.pkgname) + ".t_" + KotlinFromName(typ.name) +
					"\n" + indent + "  }"
...
func KotlinFilesFromProjectCmd(project *vxproject, command *vxcommand) ([]*vxfile, *vxmsgblock) {
	msgblock := NewMsgBlock("KotlinFilesFromProjectCmd")
	var files []*vxfile
	...
	case ":source":
		...
		file.name = "App.kt"
		file.path = cmdpath
		file.text = KotlinApp(project, command, project.javadomain)
		files = append(files, file)
	case ":test":
		file := NewFile()
		file.name = "AppTest.kt"
		file.path = cmdpath
		file.text = KotlinAppTest(project, command, project.javadomain)
		files = append(files, file)
		file = NewFile()
		file.name = "TestLib.kt"
		file.path = cmdpath
		file.text = KotlinTestLib()
		files = append(files, file)
	}
	...
	for _, pkg := range pkgs {
		...
		switch command.code {
		case ":source":
			text, msgs := KotlinFromPackage(pkg, project, javadomain)
			msgblock = MsgblockAddBlock(msgblock, msgs)
			file := NewFile()
			file.name = pkgname + ".kt"
			file.path = cmdpath + "/" + javadomainpath + "/" + pkgpath
			file.text = text
			files = append(files, file)
		case ":test":
			text, msgs := KotlinTestFromPackage(pkg, project, javadomain)
			msgblock = MsgblockAddBlock(msgblock, msgs)
			file := NewFile()
			file.name = pkgname + "Test.kt"
			file.path = cmdpath + "/" + javadomainpath + "/" + pkgpath
			file.text = text
			files = append(files, file)
		}
...
func KotlinFromConst(cnst *vxconst, pkg *vxpackage) (string, string, *vxmsgblock) {
	msgblock := NewMsgBlock("KotlinFromConst")
	var output = ""
	var doc = ""
	...
	doc += "{" + cnsttype.name + "}"
	cnstname := KotlinFromName(cnst.alias)
	cnstclassname := "Const_" + cnstname
	cnsttypename := KotlinNameFromType(cnst.vxtype)
	cnsttypeclassname := KotlinNameTypeFullFromType(cnsttype)
	initval := ""
	const_new := ""
	...
	default:
		...
		case ":list":
			clstext, msgs := KotlinFromValue(cnst.value, cnst.pkgname, emptyfunc, 3, true, false, path)
			msgblock = MsgblockAddBlock(msgblock, msgs)
			if clstext != "" {
		...
		case ":struct":
			...
			if clstext != "" {
				...
				for _, prop := range ListPropertyTraitFromType(cnst.vxtype) {
					const_new += "" +
						"\n      output.vx_p_" + KotlinFromName(prop.name) + " = val." + KotlinFromName(prop.name) + "();"

				}
			}
		}
	}
	extends := KotlinNameClassFullFromType(cnsttype)
	output += "" +
		"\n  /**" +
		"\n   * " + StringFromStringIndent(doc, "   * ") +
		"\n   */" +
		...
		"\n        \"" + cnst.name + "\", // name" +
		"\n        " + KotlinTypeDefFromType(cnsttype, "        ") +
		"\n      );" +
		"\n    }" +
		"\n" +
...
func KotlinFromFunc(fnc *vxfunc) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("KotlinFromFunc")
	var listargtype []string
	var listargname []string
	funcname := KotlinFromName(fnc.alias) + KotlinIndexFromFunc(fnc)
	returntype := ""
	if fnc.generictype == nil {
		returntype = KotlinGenericFromType(fnc.vxtype)
	} else {
		returntype = KotlinGenericFromType(fnc.generictype)
	}
	pkgname := KotlinNameFromPkgName(fnc.pkgname)
	instancevars := ""
	staticvars := ""
	instancefuncs := ""
	staticfuncs := ""
	f_suppresswarnings := ""
	path := fnc.pkgname + "/" + fnc.name + KotlinIndexFromFunc(fnc)
	genericdefinition := KotlinGenericDefinitionFromFunc(fnc)
	if fnc.isgeneric {
		switch NameFromFunc(fnc) {
		case "vx/core/copy", "vx/core/empty", "vx/core/new":
		...
		default:
			if fnc.generictype != nil {
				genericargname := KotlinNameTFromTypeGeneric(fnc.generictype)
				argtext := "final " + KotlinGenericFromType(fnc.generictype) + " " + genericargname
				listargtype = append(listargtype, argtext)
	...
	default:
		for _, arg := range fnc.listarg {
			...
			if fnc.generictype != nil && argtype.isgeneric {
				argtypename = KotlinGenericFromType(argtype)
			} else {
				argtypename = KotlinNameTypeFromType(argtype)
			}
			argtext := "final " + argtypename + " " + KotlinFromName(arg.alias)
			listargname = append(listargname, KotlinFromName(arg.alias))
			if arg.multi {
				listargtype = append(listargtype, argtext)
			} else {
				listargtype = append(listargtype, argtext)
	...
	functypetext := ""
	if fnc.generictype != nil {
		functypetext = KotlinGenericFromType(fnc.generictype)
	} else {
	...
	default:
		...
		default:
			...
			case 1:
				argtypename := ""
				arg := fnc.listarg[0]
				argtypename = KotlinNameTypeFromType(arg.vxtype)
				subargnames := "inputval"
				switch NameFromFunc(fnc) {
				case "vx/core/empty":
				default:
					if fnc.generictype != nil {
						subargnames = KotlinNameTFromType(fnc.vxtype) + ", inputval"
					}
				}
				contextname := ""
				if fnc.context {
				...
				if fnc.async {
					...
					if issamegeneric {
						...
						asyncbody += "" +
							"\n      " + KotlinNameTypeFromType(arg.vxtype) + " inputval = Core.f_any_from_any(" + KotlinNameTFromType(arg.vxtype) + ", value);" +
	...
	repltext := KotlinReplFromFunc(fnc)
	instancefuncs += repltext
	valuetext, msgs := KotlinFromValue(fnc.value, fnc.pkgname, fnc, 0, true, false, path)
	msgblock = MsgblockAddBlock(msgblock, msgs)
	...
	permissionbottom := ""
	if fnc.permission {
		permissiontop = lineindent + "if (Core.f_boolean_permission_from_func(" + KotlinNameTFromFunc(fnc) + ", context).vx_boolean()) {"
		permissionbottom = "" +
			lineindent + "} else {" +
	...
	interfacefn := KotlinInterfaceFnFromFunc(fnc)
	debugtop, debugbottom := KotlinDebugFromFunc(fnc, lineindent)
	switch NameFromFunc(fnc) {
	case "vx/core/new":
		f_suppresswarnings = "\n  @SuppressWarnings(\"unchecked\")"
	}
	...
	default:
		if fnc.vxtype.name == "none" {
			...
			defaultvalue = lineindent + "CompletableFuture<" + returntype + "> output = Core.async_new_completed(" + KotlinNameEFromType(fnc.vxtype) + ");"
		} else if fnc.generictype != nil {
			defaultvalue = lineindent + returntype + " output = Core.f_empty(" + KotlinNameTFromTypeGeneric(fnc.generictype) + ");"
		} else {
			defaultvalue = lineindent + returntype + " output = " + KotlinNameEFromType(fnc.vxtype) + ";"
		}
	}
	interfacefunc := KotlinInterfaceFromFunc(fnc)
	doc := KotlinDocFromFunc(fnc)
	output := "" +
		doc +
		interfacefunc +
		...
		"\n        " + KotlinTypeDefFromType(fnc.vxtype, "        ") + " // typedef" +
		"\n      );" +
		"\n    }" +
		"\n" +
		...
		"\n  }" +
		"\n" +
		"\n  public static final Func_" + funcname + " e_" + funcname + " = new " + KotlinNameFromPkgName(fnc.pkgname) + ".Class_" + funcname + "();" +
		"\n  public static final Func_" + funcname + " t_" + funcname + " = new " + KotlinNameFromPkgName(fnc.pkgname) + ".Class_" + funcname + "();" +
		"\n" +
		f_suppresswarnings +
...
func KotlinFuncDefsFromFuncs(funcs []*vxfunc, indent string) string {
	output := "null"
	lineindent := "\n" + indent
	if len(funcs) > 0 {
...
func KotlinFromName(name string) string {
	output := name
	if output == "extends" {
		output = "extend"
	}
...
func KotlinFromPackage(pkg *vxpackage, prj *vxproject, pkgprefix string) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("KotlinFromPackage")
	pkgpath, pkgname := KotlinPackagePathFromPrefixName(pkgprefix, pkg.name)
	specialcode := prj.maptext[pkg.name+"_kotlin.txt"]
	typkeys := ListKeyFromMapType(pkg.maptype)
	typetexts := ""
	for _, typid := range typkeys {
		typ := pkg.maptype[typid]
		typetext, msgs := KotlinFromType(typ)
		msgblock = MsgblockAddBlock(msgblock, msgs)
		typetexts += typetext
	}
	...
	for _, cnstid := range cnstkeys {
		...
		consttext, constlate, msgs := KotlinFromConst(cnst, pkg)
		msgblock = MsgblockAddBlock(msgblock, msgs)
	...
	for _, fncid := range fnckeys {
		...
		for _, fnc := range fncs {
			fnctext, msgs := KotlinFromFunc(fnc)
			msgblock = MsgblockAddBlock(msgblock, msgs)
			functexts += fnctext
		}
	}
	body := "" +
		...
		constlates
		//		emptytypes
	imports := KotlinImportsFromPackage(pkg, pkgprefix, body, false)
	output := "" +
		"package " + pkgpath + ";" +
		"\n" +
		imports +
...
func KotlinFromText(text string) string {
	var output = text
...
func KotlinFromType(typ *vxtype) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("KotlinFromType")
	output := ""
	...
	typename := KotlinFromName(typ.alias)
	instancefuncs := ""
	createtext, msgs := KotlinFromValue(typ.createvalue, "", emptyfunc, 2, true, false, path)
	msgblock = MsgblockAddBlock(msgblock, msgs)
	if createtext != "" {
		instancefuncs += "\n    " + createtext + "\n"
	}
	...
	case ":list":
		...
		if len(allowtypes) > 0 {
			...
			allowclass = KotlinNameTypeFullFromType(allowtype)
			allowempty := KotlinNameEFromType(allowtype)
			allowname = KotlinNameFromType(allowtype)
			allowcode = "" +
		...
		for _, allowedtype := range typ.allowtypes {
			allowedtypename := KotlinNameTypeFromType(allowedtype)
	...
	case ":struct":
		...
		default:
			...
			for _, arg := range props {
				...
				argname := KotlinFromName(arg.name)
				...
				vx_any += "" +
					...
					"\n        output = this." + argname + "();" +
					"\n        break;"
				argclassname := KotlinNameTypeFromType(arg.vxtype)
				argalt := ""
				...
				instancefuncs += "" +
					...
					"\n      return this.vx_p_" + argname + " == null ? " + KotlinNameEFromType(arg.vxtype) + " : this.vx_p_" + argname + ";" +
					"\n    }" +
					"\n"
			}
			defaultkey := ""
			...
			if lastarg.isdefault {
				lastargname := KotlinFromName(lastarg.name)
				argclassname := KotlinNameTypeFromType(lastarg.vxtype)
				defaultkey += "" +
				...
				if lastarg.vxtype.extends == ":list" {
					for _, allowtype := range lastarg.vxtype.allowtypes {
						subargclassname := KotlinNameTypeFromType(allowtype)
						defaultkey += "" +
							...
							"\n            if (vallist == null) {" +
							"\n              vallist = " + KotlinNameTFromType(lastarg.vxtype) + ".vx_new(valdefault);" +
							"\n            } else {" +
	...
	sinterface := KotlinInterfaceFromType(typ)
	typedef := "" +
		"\n    override public Core.Type_typedef vx_typedef() {" +
		"\n      return " + KotlinTypeDefFromType(typ, "      ") + ";" +
		"\n    }" +
		"\n"
	output += "" +
		"\n  /**" +
		...
		"\n   */" +
		sinterface +
		"\n  public static class Class_" + typename + extendsclass + " implements Type_" + KotlinNameFromType(typ) + " {" +
		"\n" +
		instancefuncs +
...
func KotlinFromValue(value vxvalue, pkgname string, parentfn *vxfunc, indent int, encode bool, test bool, path string) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("KotlinFromValue")
	var output = ""
	valstr := ""
	...
	case ":arg":
		...
		valstr = KotlinFromName(valarg.name)
		output += valstr
	case ":const":
		switch value.name {
		case "false", "true":
		...
		default:
			if value.pkg == ":native" {
				valstr = KotlinFromName(value.name)
			} else {
				valconst := ConstFromValue(value)
				valstr = KotlinNameFromPkgName(valconst.pkgname) + ".c_" + KotlinFromName(valconst.alias)
			}
		}
		output = valstr
	case ":func":
			...
			for _, arg := range fnc.listarg {
				...
				if valuetext == ":kotlin" {
					...
					if argvalue.name == "newline" {
						...
						clstext, msgs := KotlinFromValue(argvalue, pkgname, parentfn, 0, false, test, subpath)
						msgblock = MsgblockAddBlock(msgblock, msgs)
						argtext = clstext
		...
		default:
			...
			case "vx/core/any<-struct":
				...
				case "string":
					...
					work, msgs := KotlinFromValue(structvalue, pkgname, fnc, 0, true, test, subpath)
					msgblock = MsgblockAddBlock(msgblock, msgs)
					work = work + "." + KotlinFromName(propname) + "()"
			...
			case "vx/core/let":
				if fnc.async {
					output += KotlinNameFromPkgName(fnc.pkgname) + ".f_let_async("
				} else {
					output += KotlinNameFromPkgName(fnc.pkgname) + ".f_" + KotlinNameFromFunc(fnc) + "("
				}
			default:
				output += KotlinNameFromPkgName(fnc.pkgname) + ".f_" + KotlinNameFromFunc(fnc) + "("
			}
			if !isskip {
				if fnc.isgeneric {
					...
					default:
						if fnc.generictype != nil {
							genericarg := KotlinNameTFromTypeGeneric(fnc.vxtype)
					...
					if fncidx == 0 && funcname == "vx/core/fn" {
						...
						if argvalue.code == ":func" && argvalue.name == "fn" {
							...
							for _, lambdaarg := range arglist {
								...
								argvaltype = KotlinNameTypeFullFromType(argtype)
								argvalinstance = KotlinNameTFromType(argtype)
							...
							if argfunc.async {
								work, msgs := KotlinFromValue(argvalue, pkgname, fnc, subindent, true, test, argsubpath)
								msgblock = MsgblockAddBlock(msgblock, msgs)
								work = "\n  return " + work + ";"
								...
								case "any<-key-value-async",
									...
									argtext = "" +
										KotlinNameTFromType(funcarg.vxtype) + ".vx_fn_new((" + lambdatext + ") -> {" +
										lambdavartext +
										work +
										"\n})"
								default:
							...
							case 1:
								...
								if argasync {
									...
									for _, lambdaarg := range arglist {
										...
										if lambdaarg.async {
											lambdavaluetext, msgs := KotlinFromValue(lambdaarg.value, pkgname, fnc, argindent, true, test, argsubpath)
											msgblock = MsgblockAddBlock(msgblock, msgs)
											lambdatext += "" +
												arglineindent + "final CompletableFuture<" + KotlinNameTypeFromType(lambdaarg.vxtype) + "> future_" + KotlinFromName(lambdaarg.name) + " = " + lambdavaluetext + ";" +
												arglineindent + "return Core.async_from_async_fn(future_" + KotlinFromName(lambdaarg.name) + ", (" + KotlinFromName(lambdaarg.name) + ") -> {"
											aftertext += "" +
												arglineindent + "});"
											argindent += 1
										} else {
											lambdavaluetext, msgs := KotlinFromValue(lambdaarg.value, pkgname, fnc, argindent, true, test, argsubpath)
											msgblock = MsgblockAddBlock(msgblock, msgs)
											lambdatext += arglineindent + "final " + KotlinNameTypeFromType(lambdaarg.vxtype) + " " + KotlinFromName(lambdaarg.name) + " = " + lambdavaluetext + ";"
										}
									}
									work, msgs := KotlinFromValue(argvalue, pkgname, fnc, 2, true, test, argsubpath)
									msgblock = MsgblockAddBlock(msgblock, msgs)
									argtext = "" +
									...
									for _, lambdaarg := range arglist {
										...
										lambdavaluetext, msgs := KotlinFromValue(lambdaarg.value, pkgname, fnc, argindent, true, test, lambdaargpath)
										msgblock = MsgblockAddBlock(msgblock, msgs)
										lambdatext += arglineindent + "final " + KotlinNameTypeFromType(lambdaarg.vxtype) + " " + KotlinFromName(lambdaarg.name) + " = " + lambdavaluetext + ";"
									}
									work, msgs := KotlinFromValue(argvalue, pkgname, fnc, 0, true, test, argsubpath)
									msgblock = MsgblockAddBlock(msgblock, msgs)
							...
							case ":funcref":
								...
								case "any<-key-value", "any<-key-value-async",
									...
									lambdatext, lambdavartext, lambdaargtext := KotlinLambdaFromArgList(funcarglist, funcargfunc.isgeneric)
									work := KotlinNameFFromFunc(funcargfunc) + "(" + lambdaargtext + ")"
							...
							default:
								...
								work, msgs := KotlinFromValue(argvalue, pkgname, fnc, workindent, true, test, argsubpath)
								if converttoasync {
									sworkindent := StringRepeat("  ", workindent)
									work = "Core.f_async(" + KotlinNameTFromType(argfunctype) + ",\n" + sworkindent + work + "\n  )"
								}
								msgblock = MsgblockAddBlock(msgblock, msgs)
								...
								argtext = "" +
									KotlinNameTFromType(funcarg.vxtype) + ".vx_fn_new(() -> {" +
									work +
									"\n})"
							}
						}
						...
						if funcarg.multi {
							...
							if ismultiarg {
								...
								multiflag = true
								argtext = "" +
									KotlinNameTFromType(funcarg.vxtype) + ".vx_new(" +
	...
	case ":funcref":
		...
		valstr = ""
		valstr += KotlinNameFromPkgName(valfunc.pkgname) + ".t_" + KotlinFromName(valfunc.alias)
		output = sindent + valstr
	case ":type":
		valtype := TypeFromValue(value)
		output = KotlinNameFromPkgName(valtype.pkgname) + ".t_" + KotlinFromName(valtype.alias)
	case "string":
		...
		if encode {
			output = KotlinTypeStringValNew(output)
		}
	case "boolean":
		if encode {
			valstr = StringValueFromValue(value)
...
func KotlinGenericDefinitionFromFunc(fnc *vxfunc) string {
	output := ""
	var mapgeneric = make(map[string]string)
	if fnc.generictype != nil {
		returntype := KotlinGenericFromType(fnc.generictype)
		mapgeneric[fnc.vxtype.name] = returntype + " extends " + KotlinNameTypeFromType(fnc.vxtype)
		for _, arg := range fnc.listarg {
			argtype := arg.vxtype
			if !argtype.isfunc {
				if argtype.isgeneric {
					...
					if !ok {
						argtypename := KotlinGenericFromType(argtype)
						worktext := argtypename + " extends " + KotlinNameTypeFromType(arg.vxtype)
...
func KotlinGenericFromType(typ *vxtype) string {
	output := ""
	if typ.isgeneric {
		switch typ.name {
		case "any-1":
		...
		output = KotlinNameTypeFromType(typ)
	}
	return output
}

func KotlinImportsFromPackage(pkg *vxpackage, pkgprefix string, body string, test bool) string {
	output := ""
...
func KotlinIndexFromFunc(fnc *vxfunc) string {
	return StringIndexFromFunc(fnc)
}

func KotlinInterfaceFromType(typ *vxtype) string {
	output := ""
	typename := KotlinNameTypeFullFromType(typ)
	basics := "" +
	...
	createtext, _ := KotlinFromValue(typ.createvalue, "", emptyfunc, 0, true, false, "")
	if createtext != "" {
		...
		case ":struct":
			...
			if len(typ.traits) > 0 {
				...
				for _, trait := range typ.traits {
					traitnames = append(traitnames, KotlinNameTypeFullFromType(trait))
				}
				extends = StringFromListStringJoin(traitnames, ", ")
			}
			for _, arg := range ListPropertyTraitFromType(typ) {
				extras += "\n    public " + KotlinNameTypeFromType(arg.vxtype) + " " + KotlinFromName(arg.alias) + "();"
			}
		default:
			extends += "Core.Type_any"
		}
		typename := KotlinNameFromType(typ)
		output = "" +
...
func KotlinInterfaceFnFromFunc(fnc *vxfunc) string {
	interfaces := ""
	switch NameFromFunc(fnc) {
	case "vx/core/any<-any", "vx/core/any<-any-async",
		"vx/core/any<-any-context", "vx/core/any<-any-context-async",
		...
		for _, arg := range fnc.listarg {
			argtype := arg.vxtype
			argname := KotlinFromName(arg.alias)
			argtypename := KotlinNameTypeFromType(argtype)
...
func KotlinInterfaceFromFunc(fnc *vxfunc) string {
	funcname := KotlinNameFromFunc(fnc)
	extends := ""
	interfaces := ""
	generictypes := KotlinGenericDefinitionFromFunc(fnc)
	returntype := KotlinNameTypeFromType(fnc.vxtype)
...
func KotlinLambdaFromArgList(arglist []vxarg, isgeneric bool) (string, string, string) {
	...
	for _, lambdaarg := range arglist {
		argvaltype := ""
		argtype := lambdaarg.vxtype
		lambdaargname := KotlinFromName(lambdaarg.alias)
		...
		default:
			argvaltype = KotlinNameTypeFullFromType(argtype)
			argvaltname := KotlinNameTFromType(argtype)
...
func KotlinNameClassFullFromType(typ *vxtype) string {
	name := KotlinNameFromPkgName(typ.pkgname)
	name += ".Class_"
	name += KotlinNameFromType(typ)
	return name
}

func KotlinNameEFromType(typ *vxtype) string {
	output := ""
	if typ.isgeneric {
		output = "Core.f_empty(generic_" + KotlinFromName(typ.name) + ")"
	} else {
		output = "e_" + KotlinNameFromType(typ)
		if typ.pkgname != "" {
			output = KotlinNameFromPkgName(typ.pkgname) + "." + output
		}
	}
	return output
}

func KotlinNameFFromFunc(fnc *vxfunc) string {
	name := "f_" + KotlinNameFromFunc(fnc)
	if fnc.pkgname != "" {
		name = KotlinNameFromPkgName(fnc.pkgname) + "." + name
	}
	return name
}

func KotlinNameFromFunc(fnc *vxfunc) string {
	return KotlinFromName(fnc.alias) + KotlinIndexFromFunc(fnc)
}

func KotlinNameFromPkgName(pkgname string) string {
	output := ""
...
func KotlinNameFromType(typ *vxtype) string {
	name := ""
	if typ.alias == "" {
		name += KotlinFromName(typ.name)
	} else {
		name += KotlinFromName(typ.alias)
	}
	return name
}

func KotlinNameTFromFunc(fnc *vxfunc) string {
	name := "t_" + KotlinNameFromFunc(fnc)
	if fnc.pkgname != "" {
		name = KotlinNameFromPkgName(fnc.pkgname) + "." + name
	}
	return name
...
func KotlinNameTFromType(typ *vxtype) string {
	name := "t_" + KotlinNameFromType(typ)
	if typ.pkgname != "" {
		name = KotlinNameFromPkgName(typ.pkgname) + "." + name
	}
	return name
}

func KotlinNameTFromTypeGeneric(typ *vxtype) string {
	name := ""
	if typ.isgeneric {
		name = "generic_" + KotlinFromName(typ.name)
	} else {
		name = KotlinNameTFromType(typ)
	}
	return name
}

func KotlinNameTypeFromType(typ *vxtype) string {
	name := KotlinNameTypeFullFromType(typ)
	switch name {
	case "Core.Type_none":
		name = "void"
	}
...
func KotlinNameTypeFullFromType(typ *vxtype) string {
	name := KotlinNameFromPkgName(typ.pkgname)
	if typ.isfunc {
		name += ".Func_"
	} else {
...
func KotlinPackagePathFromPrefixName(pkgprefix string, pkgname string) (string, string) {
	...
	pkgpath = KotlinFromName(pkgpath)
	name = KotlinNameFromPkgName(name)
	return pkgpath, name
}

func KotlinReplFromFunc(fnc *vxfunc) string {
	output := ""
	replparams := ""
	argidx := 0
	var listargname []string
	...
	default:
		outputtype = KotlinNameTypeFromType(fnc.vxtype)
		outputttype = KotlinNameTFromType(fnc.vxtype)
		//emptytype = KotlinNameEFromType(fnc.vxtype)
		returnvalue = "output = "
	}
	if fnc.isgeneric {
		switch NameFromFunc(fnc) {
		...
		default:
			if fnc.generictype != nil {
				replparam := outputtype + " generic_" + KotlinFromName(fnc.generictype.name) + " = Core.f_any_from_any(" + outputttype + ", arglist.vx_any(Core.vx_new_int(" + StringFromInt(argidx) + ")));"
				replparams += "\n      " + replparam
				listargname = append(listargname, "generic_"+KotlinFromName(fnc.generictype.name))
			}
		}
	}
	for _, arg := range fnc.listarg {
		if (funcname == "let" || funcname == "let_async") && arg.name == "args" {
			...
			argname := KotlinFromName(arg.alias)
			replparam := KotlinNameTypeFromType(arg.vxtype) + " " + argname + " = Core.f_any_from_any(" + KotlinNameTFromType(arg.vxtype) + ", arglist.vx_any(Core.vx_new_int(" + StringFromInt(argidx) + ")));"
			replparams += "\n      " + replparam
...
func KotlinStringFromProjectCmd(prj *vxproject, cmd *vxcommand) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("KotlinStringFromProjectCmd")
	files, msgs := KotlinFilesFromProjectCmd(prj, cmd)
	msgblock = MsgblockAddBlock(msgblock, msgs)
	text := StringFromListFile(files)
	return text, msgblock
}

func KotlinTestCase(testvalues []vxvalue, testpkg string, testname string, testcasename string, fnc *vxfunc, path string) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("KotlinTestCase")
	var output = ""
	if len(testvalues) > 0 {
		var desctexts []string
		for idx, testvalue := range testvalues {
			subpath := path + "/tests" + StringFromInt(idx+1)
			descvaluetext, msgs := KotlinFromValue(testvalue, testpkg, fnc, 6, true, true, subpath)
			msgblock = MsgblockAddBlock(msgblock, msgs)
			desctext := "" +
				"\n        Test.t_testdescribe.vx_new(" +
				"\n          \":describename\", \"" + KotlinTestFromValue(testvalue) + "\"," +
				"\n          \":testresult\"," +
				"\n            " + descvaluetext +
				"\n        )"
...
func KotlinTestFromConst(cnst *vxconst) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("KotlinTestFromConst")
	testvalues := cnst.listtestvalue
	testpkg := cnst.pkgname
	testname := cnst.name
	testcasename := "c_" + KotlinFromName(cnst.alias)
	path := cnst.pkgname + "/" + cnst.name
	fnc := emptyfunc
	output, msgs := KotlinTestCase(testvalues, testpkg, testname, testcasename, fnc, path)
	msgblock = MsgblockAddBlock(msgblock, msgs)
	return output, msgblock
}

func KotlinTestFromFunc(fnc *vxfunc) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("KotlinTestFromFunc")
	testvalues := fnc.listtestvalue
	testpkg := fnc.pkgname
	idx := KotlinIndexFromFunc(fnc)
	testname := fnc.name + idx
	funcname := KotlinFromName(fnc.alias) + idx
...
func KotlinTestFromPackage(pkg *vxpackage, prj *vxproject, pkgprefix string) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("KotlinTestFromPackage")
	pkgpath, pkgname := KotlinPackagePathFromPrefixName(pkgprefix, pkg.name)
	typkeys := ListKeyFromMapType(pkg.maptype)
	var coverdoccnt = 0
	var coverdoctotal = 0
	...
	for _, typid := range typkeys {
		covertypetotal += 1
		typ := pkg.maptype[typid]
		test, msgs := KotlinTestFromType(typ)
		msgblock = MsgblockAddBlock(msgblock, msgs)
		covertype = append(covertype, "      \":"+typid+"\", "+StringFromInt(len(typ.testvalues)))
		if test != "" {
			covertypecnt += 1
			typetexts += test
			testall = append(testall, pkgname+"Test.t_"+KotlinFromName(typ.alias)+"(context)")
		}
		coverdoctotal += 1
		if typ.doc != "" {
			coverdoccnt += 1
	...
	for _, cnstid := range cnstkeys {
		coverconsttotal += 1
		cnst := pkg.mapconst[cnstid]
		test, msgs := KotlinTestFromConst(cnst)
		msgblock = MsgblockAddBlock(msgblock, msgs)
		coverconst = append(coverconst, "      \":"+cnstid+"\", "+StringFromInt(len(cnst.listtestvalue)))
		if test != "" {
			coverconstcnt += 1
			consttexts += test
			testall = append(testall, pkgname+"Test.c_"+KotlinFromName(cnst.alias)+"(context)")
		}
		coverdoctotal += 1
		if cnst.doc != "" {
			coverdoccnt += 1
	...
	for _, fncid := range fnckeys {
		...
		fncs := pkg.mapfunc[fncid]
		for _, fnc := range fncs {
			test, msgs := KotlinTestFromFunc(fnc)
			msgblock = MsgblockAddBlock(msgblock, msgs)
			msgblock = MsgblockAddBlock(msgblock, msgs)
			coverfunc = append(coverfunc, "      \":"+fncid+KotlinIndexFromFunc(fnc)+"\", "+StringFromInt(len(fnc.listtestvalue)))
			if test != "" {
				coverfunccnt += 1
				functexts += test
				testall = append(testall, pkgname+"Test.f_"+KotlinFromName(fnc.alias)+KotlinIndexFromFunc(fnc)+"(context)")
			}
			coverdoctotal += 1
			if fnc.doc != "" {
				coverdoccnt += 1
	...
	body := "" +
		...
		"\n      \":testpkg\",   \"" + pkg.name + "\", " +
		"\n      \":constnums\", " + KotlinTypeCoverageNumsValNew(coverconstpct, coverconstcnt, coverconsttotal) + ", " +
		"\n      \":docnums\", " + KotlinTypeCoverageNumsValNew(coverdocpct, coverdoccnt, coverdoctotal) + ", " +
		"\n      \":funcnums\", " + KotlinTypeCoverageNumsValNew(coverfuncpct, coverfunccnt, coverfunctotal) + ", " +
		"\n      \":ospacenums\", " + KotlinTypeCoverageNumsValNew(coverbigospacepct, coverbigospacecnt, coverbigospacetotal) + ", " +
		"\n      \":otimenums\", " + KotlinTypeCoverageNumsValNew(coverbigotimepct, coverbigotimecnt, coverbigotimetotal) + ", " +
		"\n      \":totalnums\", " + KotlinTypeCoverageNumsValNew(coverpct, covercnt, covertotal) + ", " +
		"\n      \":typenums\", " + KotlinTypeCoverageNumsValNew(covertypepct, covertypecnt, covertypetotal) +
		"\n    );" +
		"\n  }" +
		"\n" +
		...
		"\n  }" +
		"\n"
	imports := KotlinImportsFromPackage(pkg, pkgprefix, body, true)
	output := "" +
		"\npackage " + pkgpath + ";" +
		"\n" +
		imports +
...
func KotlinTestFromType(typ *vxtype) (string, *vxmsgblock) {
	msgblock := NewMsgBlock("KotlinTestFromType")
	testvalues := typ.testvalues
	testpkg := typ.pkgname
	testname := typ.name
	testcasename := "t_" + KotlinFromName(typ.alias)
	fnc := emptyfunc
	path := typ.pkgname + "/" + typ.name
	output, msgs := KotlinTestCase(testvalues, testpkg, testname, testcasename, fnc, path)
	msgblock = MsgblockAddBlock(msgblock, msgs)
...
func KotlinTestFromValue(value vxvalue) string {
	var output = ""
	output = KotlinFromText(value.textblock.text)
	return output
}

func KotlinTypeCoverageNumsValNew(pct int, tests int, total int) string {
	return "" +
...
func KotlinTypeDefFromType(typ *vxtype, indent string) string {
	lineindent := "\n" + indent
	allowtypes := KotlinTypeListFromListType(typ.allowtypes)
	disallowtypes := KotlinTypeListFromListType(typ.disallowtypes)
	...
	traits := KotlinTypeListFromListType(typ.traits)
	output := "" +
		"Core.typedef_new(" +
...
func KotlinTypeDefMapFromProperties(args []vxarg, indent string) string {
	output := "null"
	lineindent := "\n" + indent
	if len(args) > 0 {
		var props []string
		for _, arg := range args {
			key := KotlinFromName(arg.alias)
			typ := KotlinTypeDefFromType(arg.vxtype, indent+"  ")
...
func KotlinTypeIntValNew(val int) string {
	return "Core.vx_new_int(" + StringFromInt(val) + ")"
}

func KotlinTypeListFromListType(listtype []*vxtype) string {
	output := "Core.e_typelist"
	if len(listtype) > 0 {
		var listtext []string
		for _, typ := range listtype {
			typetext := KotlinNameTFromType(typ)
...
func KotlinTypeStringValNew(val string) string {
	valstr := StringFromStringFindReplace(val, "\n", "\\n")
	return "Core.vx_new_string(\"" + valstr + "\")"
}

func KotlinWriteFromProjectCmd(prj *vxproject, cmd *vxcommand) *vxmsgblock {
	msgblock := NewMsgBlock("KotlinWriteFromProjectCmd")
	files, msgs := KotlinFilesFromProjectCmd(prj, cmd)
	msgblock = MsgblockAddBlock(msgblock, msgs)
	...
	case ":test":
		...
		msgs := KotlinFolderCopyTestdataFromProjectPath(prj, targetpath)
		msgblock = MsgblockAddBlock(msgblock, msgs)
	}
	return msgblock
}

func KotlinApp(project *vxproject, cmd *vxcommand, pkgprefix string) string {
	includetext := ""
	contexttext := `
	...
	if cmd.context == "" && cmd.main == "" {
		...
		if contextfunc != emptyfunc {
			...
			if contextfunc.async {
				contexttext = `
      CompletableFuture<Core.Type_context> asynccontext = ` + KotlinNameFFromFunc(contextfunc) + `(arglist);
      Core.Type_context context = Core.vx_sync_from_async(vx_core::t_context, asynccontext);`
			} else {
				contexttext = `
      Core.Type_context context = ` + KotlinNameFFromFunc(contextfunc) + `(arglist);`
			}
		}
		if mainfunc != emptyfunc {
			...
			mainfunctext := KotlinNameFFromFunc(mainfunc) + "(" + params + ");"
			if mainfunc.async {
				maintext = `
...
fun main(args : Array<String>) {
  try {
    var output : String = ""
    var arglist : vx_core.Type_anylist = vx_core.vx_anylist_from_arraystring(args)` +
		contexttext +
		maintext + `
...
func KotlinAppTest(project *vxproject, cmd *vxcommand, pkgprefix string) string {
	imports := ""
	contexttext := `
	...
	if cmd.context == "" {
		...
		if contextfunc != emptyfunc {
			...
			if contextfunc.async {
				contexttext = `
  Core.Type_anylist arglist = Core.e_anylist;
  CompletableFuture<Core.Type_context> asynccontext = ` + pkgpath + `.` + KotlinNameFFromFunc(contextfunc) + `(arglist);
  Core.Type_context context = Core.vx_sync_from_async(vx_core::t_context, asynccontext);`
			} else {
				contexttext = `
  Core.Type_anylist arglist = Core.e_anylist;
  Core.Type_context context = ` + pkgpath + `.` + KotlinNameFFromFunc(contextfunc) + `(arglist);`
			}
		}
	}
	tests := ""
