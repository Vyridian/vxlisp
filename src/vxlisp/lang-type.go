...
func LangType(
		...
		instancefuncs += "" +
			LangVarProp(lang, 2, rawlisttype, allowtype,
				...
				LangFuncCallVx(lang, 2,
					func_listimmutable, LangVarListNew(lang, allowtype, ""))) +
			"\n" +
			LangTypeVxList(lang, path,
				typ, false) +
			LangTypeVxAllowList(lang, path,
				typ, false)
	case ":map":
		allowtype := anytype
		allowname := "any"
		...
		instancefuncs += "" +
			LangVarProp(lang, 2, rawmaptype, allowtype,
				"vx_p_map",
				LangValMapNew(lang, 2,
					allowtype, "")) +
			"\n" +
			LangNativeTypeVxMap(lang, path,
				allowtype) +
			LangTypeVxSet(lang, path,
				typ) +
			LangTypeVxAllowMap(lang, path,
				typ, false) +
			LangTypeVxNewMap(lang, path,
				typ, false)
	case ":struct":
		vx_any := ""
		valcopyend := ""
			...
			for _, arg := range props {
						...
						LangIfElse(lang, 6,
							LangIfThen(lang, 7,
								...
								LangVarMapSet(lang, 7,
									"mapany",
									"\"value\"", "msgval")+
								LangVal(lang, 7, maptype,
									"msgmap",
									LangFuncCallMethod(lang, 7,
										LangTypeT(lang, anymaptype),
										"vx_new_from_map",
										LangFuncCallVx(lang, 8,
											func_mapimmutable,
											"mapany")))+
				...
				instancefuncs += "" +
					LangVarProp(lang, 2, arg.vxtype, emptytype,
						"vx_p_"+argname, "null") +
					"\n" +
					LangVxArgFromArg(lang, path,
						LangNameFromType(lang, typ), arg)
			}
			defaultkey := ""
			defaultstring := ""
	...
	output = "" +
		doc +
		LangTypeInterface(lang, path, typ) +
		LangTypeClassHeader(lang, 1, typ) +
		"\n" +
		instancefuncs +
		LangTypeVxNew(lang, path, typ, false) +
		LangTypeVxCopy(lang, path, typ, false) +
		LangTypeVxMsgblock(lang, path, typ, false) +
		LangTypeVxEmpty(lang, path, typ, false) +
		LangTypeVxType(lang, path, typ, false) +
		LangTypeVxTypedef(lang, path, typ, false) +
		"\n  }" +
		"\n" +
		e_type +
		t_type +
...
func LangTypeDef(
	...
	typ *vxtype,
	indent int) string {
	allowtypes := LangTypeListFromListType(lang, indent+1, typ.allowtypes)
	disallowtypes := LangTypeListFromListType(lang, indent+1, typ.disallowtypes)
	allowfuncs := LangPkgNameDot(lang, "vx/core") + "e_funclist"
	disallowfuncs := LangPkgNameDot(lang, "vx/core") + "e_funclist"
	allowvalues := LangTypeE(lang, anylisttype)
	disallowvalues := LangTypeE(lang, anylisttype)
	properties := LangPkgNameDot(lang, "vx/core") + "e_argmap"
	traits := LangTypeListFromListType(lang, indent+1, typ.traits)
	output := "" +
		LangFuncCallRaw(lang, indent,
			func_typedef_new,
			"\""+typ.pkgname+"\"",
			"\""+typ.name+"\"",
			"\""+typ.extends+"\"",
...
func LangTypeEFuture(
	...
	case langswift:
		output = "" +
			...
			"vx_async_empty(" + LangTypeTGeneric(lang, typ) + ")"
	default:
		output = "" +
			LangPkgNameDot(lang, "vx/core") +
			...
			"f_empty(" + LangTypeTGeneric(lang, typ) + "))"
	}
	return output
}
...
func LangTypeInterface(
	...
	basics := LangTypeInterfaceBasics(lang, path, typ)
	...
	if createtext != "" {
		...
		for _, createline := range createlines {
			if (createline == "@Override") || (createline == "// :implements") {
				...
				interfaceline := LangTypeInterfaceCustom(lang, createline)
				if interfaceline != "" {
					basics += "\n    " + interfaceline
				}
				isoverride = false
	...
	case "vx/core/any":
		...
		output = "" +
			LangTypeInterfaceHeader(lang, anytype, emptylisttype, 1,
				basics+
					LangFuncHeaderInterface(lang, path, 2,
						typename, funcvxtypedef)+
					LangFuncHeaderInterface(lang, path, 2,
						typename, funcvxconstdef)+
					LangFuncHeaderInterface(lang, path, 2,
						typename, funcvxdispose)+
					LangTypeVxMsgblock(lang, path,
						typ, true)+
					LangFuncHeaderInterface(lang, path, 2, typename, funcvxrelease)+
					LangFuncHeaderInterface(lang, path, 2, typename, funcvxreserve))
	case "vx/core/boolean":
		output = "" +
...
func LangTypeListFromListType(
	...
	output := LangTypeE(lang, typelisttype)
	if len(listtype) > 0 {
		var listtext []string
...
func LangTypeVxAllowMap(
	...
	if len(allowtypes) > 0 {
		...
		funcvxallow.listarg = listarg
		if isinterface {
			output += LangFuncInterface(lang, path,
				funcvxallow)
		} else {
			funcvxallow.isimplement = true
			output += "" +
				LangFuncHeader(lang, path, 2,
					...
					LangVar(lang, 3, allowtype,
						...
						LangVar(lang, 3, rawstringtype,
							"skey", "key.vx_string()")+
						LangIfThen(lang, 3,
							LangVarStringStarts(lang, "skey", "\":\""),
							LangVarSet(lang, 4,
								"skey",
								LangVarSubstring(lang, "skey", "1", "")))+
						LangIfEnd(lang, 3)+
						LangValCollection(lang, 3, rawmaptype, allowtype,
							"mapval", "map.vx_p_map")+
						LangVarSet(lang, 3,
							"output", getorelse))
		}
...
func LangTypeVxCopy(
	...
	if isinterface {
		...
		case "vx/core/float":
			...
			valnew = "" +
				...
				LangIfElseType(lang, 4,
					...
					LangVarSet(lang, 5,
						"ischanged", "true")+
						LangVarFloatPlus(lang, 5, rawfloattype,
				...
				LangIfElseType(lang, 4,
					...
					LangVarSet(lang, 5,
						"ischanged", "true")+
						LangVarFloatPlus(lang, 5, rawinttype,
							"floatval", "valint.vx_int()")) +
				LangIfElseType(lang, 4,
					stringtype, emptytype, "valsub", "valstring", false,
					LangVarSet(lang, 5,
						"ischanged", "true")+
						LangVarFloatPlus(lang, 5, rawstringtype,
							"floatval", "valstring.vx_string()")) +
				LangIfElseType(lang, 4,
				...
				LangForList(lang, 3,
					...
					LangIfThen(lang, 4,
						...
						LangIfElseType(lang, 4,
							...
							LangVarSet(lang, 5,
								"ischanged", "true")+
								LangVarIntPlus(lang, 5, rawintegertype,
						...
						LangIfElseType(lang, 4,
							...
							LangVarSet(lang, 5,
								"ischanged", "true")+
								LangVarIntPlus(lang, 5, rawstringtype,
									"intval", "sval"))+
						LangIfEnd(lang, 4)) +
				LangIfThen(lang, 3,
					LangClause(lang, false,
		...
		case "vx/core/string":
			vxstringbuilder := LangTypeStringbuilder(lang, "sb")
						...
						LangIfElseType(lang, 4,
							...
							LangVar(lang, 5, rawstringtype,
								...
								LangIfElse(lang, 5,
									LangVarSet(lang, 6,
										"ischanged", "true")+
										LangVarStringBuilderPlus(lang, 6, rawstringtype,
											"sb", "ssub"))+
								LangIfEnd(lang, 5))+
						LangIfElseType(lang, 4,
							inttype, emptytype, "valsub", "valint", false,
							LangVarSet(lang, 5,
								"ischanged", "true")+
								LangVarStringBuilderPlus(lang, 5, rawintegertype,
									"sb", "valint.vx_int()"))+
						LangIfElseType(lang, 4,
							floattype, emptytype, "valsub", "valfloat", false,
							LangVarSet(lang, 5,
								"ischanged", "true")+
								LangVarStringBuilderPlus(lang, 5, rawfloattype,
									"sb", "valfloat.vx_float()"))+
						LangIfElseType(lang, 4,
							decimaltype, emptytype, "valsub", "valdecimal", false,
							LangVarSet(lang, 5,
								"ischanged", "true")+
								LangVarStringBuilderPlus(lang, 5, rawstringtype,
		...
		case ":list":
			...
			for _, allowedtype := range typ.allowtypes {
				...
				if allowedtypename == allowclass {
					...
					case "vx/core/boolean":
						allowedtypename = LangTypeBoolean(lang)
						castval = LangFuncCallVariadicVx(lang, 5,
							func_new,
							[]string{
								LangTypeT(lang, booleantype)},
							[]string{
								"valsub"})
					case "vx/core/int":
						allowedtypename = LangTypeInt(lang)
						castval = LangFuncCallVariadicVx(lang, 5,
							func_new,
							[]string{
								LangTypeT(lang, inttype)},
							[]string{
								"valsub"})
					case "vx/core/float":
						allowedtypename = LangTypeFloat(lang)
						castval = LangFuncCallVariadicVx(lang, 5,
							func_new,
							[]string{
								LangTypeT(lang, floattype)},
							[]string{
								"valsub"})
					case "vx/core/string":
						allowedtypename = LangTypeString(lang)
						castval = LangFuncCallVariadicVx(lang, 5,
							func_new,
							[]string{
								LangTypeT(lang, stringtype)},
			...
			elseiflistany := LangTypeElseIfListAny(
				lang, allowtype)
			anyinvalid := ""
			if !isallowany {
				anyinvalid =
			...
			for _, allowedtype := range typ.allowtypes {
				...
				if allowedtypename == allowclass {
					...
					case "vx/core/boolean":
						...
						castval = LangFuncCallVariadicVx(lang, 6,
							func_new,
							[]string{
								LangTypeT(lang, booleantype)},
							[]string{
				...
				LangIfElse(lang, 5,
					LangIfThen(lang, 6,
						...
						LangVarMapSet(lang, 6,
							...
							"\"value\"",
							"msgval")+
						LangVal(lang, 6, maptype,
							"msgmap",
							LangFuncCallMethod(lang, 6,
								LangTypeT(lang, anymaptype),
								"vx_new_from_map",
								LangFuncCallVx(lang, 7,
									func_mapimmutable,
									...
									LangIfElse(lang, 5,
										LangVarSet(lang, 6,
											...
											LangFuncCallVariadicVx(lang, 6,
												func_copy,
												[]string{
													LangTypeT(lang, msgblocklisttype),
													"vx_p_msgblocks"},
												[]string{
													"valsub"})))+
									LangIfEnd(lang, 5))+
								LangIfElseIf(lang, 4,
									...
									LangIfThen(lang, 5,
										LangClause(lang, false,
											"!=",
											"valsub",
											LangTypeE(lang, msgblocklisttype)),
										LangVarSet(lang, 6,
											"vx_p_msgblocks",
											LangFuncCallVariadicVx(lang, 6,
												func_copy,
												...
												LangIfThen(lang, 5,
													...
													LangIfThen(lang, 6,
														...
														LangVar(lang, 6, rawbooltype,
															"isvalidkey",
															LangStringContains(lang, "validkeys", "testkey"))+
														LangIfThen(lang, 6,
															"isvalidkey",
															LangVarSet(lang, 7,
																"key", "testkey")+
